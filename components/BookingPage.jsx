...
const BookingPage = () => {
  ...
  const [form, setForm] = useState({ fullName: '', phone: '', pickup: '', destination: '', date: '', time: '', vehicleType: 'Sedan', passengers: '1-4' });
  ...
  const validate = () => {
    ...
    else if (!phoneRegex.test(form.phone.replace(/\s/g, ''))) newErrors.phone = 'Enter a valid phone number (10–13 digits).';
    ...
    if (!form.passengers) newErrors.passengers = 'Please select number of passengers.';
    return newErrors;
  };
        ...
        {!isSubmitted ? (
          <div className="booking-card">
            ...
            <form className="booking-form-detailed" onSubmit={handleSubmit} noValidate>
              <section className="form-section">
                ...
                <div className="form-grid">
                  <div className="form-group">
                    ...
                    <input type="text" name="fullName" placeholder="John Doe" value={form.fullName} onChange={handleChange} className={errors.fullName ? 'input-error' : ''} />
                    {errors.fullName && <span className="error-msg">⚠ {errors.fullName}</span>}
                  </div>
                  <div className="form-group">
                    <label>Phone Number</label>
                    <input type="tel" name="phone" placeholder="+91 98765 43210" value={form.phone} onChange={handleChange} className={errors.phone ? 'input-error' : ''} />
              ...
              <section className="form-section">
                ...
                <div className="form-grid">
                  <div className="form-group">
                    ...
                    <input type="text" name="pickup" placeholder="Hotel / Airport / Residence" value={form.pickup} onChange={handleChange} className={errors.pickup ? 'input-error' : ''} />
                    {errors.pickup && <span className="error-msg">⚠ {errors.pickup}</span>}
                  </div>
                  <div className="form-group">
                    <label>Destination</label>
                    <input type="text" name="destination" placeholder="Where are you going?" value={form.destination} onChange={handleChange} className={errors.destination ? 'input-error' : ''} />
                    {errors.destination && <span className="error-msg">⚠ {errors.destination}</span>}
                  </div>
                  <div className="form-group">
                    <label>Pickup Date</label>
                    <input type="date" name="date" min={today} value={form.date} onChange={handleChange} className={errors.date ? 'input-error' : ''} />
                    {errors.date && <span className="error-msg">⚠ {errors.date}</span>}
                  </div>
                  <div className="form-group">
                    <label>Pickup Time</label>
                    <input type="time" name="time" value={form.time} onChange={handleChange} className={errors.time ? 'input-error' : ''} />
                    {errors.time && <span className="error-msg">⚠ {errors.time}</span>}
                  </div>
                  <div className="form-group">
                    <label>Vehicle Type</label>
                    <select name="vehicleType" value={form.vehicleType} onChange={handleChange} className={errors.vehicleType ? 'input-error' : ''}>
                  ...
                  <div className="form-group">
                    ...
                    <select name="passengers" value={form.passengers} onChange={handleChange} className={errors.passengers ? 'input-error' : ''}>
              ...
              <button type="submit" className="btn btn-primary btn-book-large">Book Now</button>
            </form>
          </div>
        ) : (
          <div className="booking-card confirmation-card animated fadeIn">
            <div className="confirmation-header">
              <div className="success-badge"><i className="fas fa-check-circle"></i></div>
